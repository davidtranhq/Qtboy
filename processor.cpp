...
Processor::Processor(Memory &mem,
	...
	: memory_ {mem}, 
	  af_ {af}, bc_ {bc}, de_ {de}, hl_ {hl}, sp_ {sp}, pc_ {pc},
	  disassembler_ {"opcodes.json"}
{}
...
void Processor::dump(std::ostream &os)
{
	...
	for (uint8_t i; i < instr_len[op]; ++i)
		ops.push_back(read(pc_+i));
	std::cout << disassembler_.disassemble(ops) << '\n';
	std::cout << std::hex << std::setfill('0')
		<< "opcode: " << std::setw(2) << static_cast<int>(read(pc_)) << '\n';
